...
const SortableKeyItem: React.FC<{ column: Column; onRemove: () => void }> = ({ column, onRemove }) => {
          ...
          <PopoverTrigger asChild>
            <Button variant="outline" role="combobox" aria-expanded={open} className="w-full flex !justify-between">
              <span className="truncate">
                {selectedColumns.length > 0 ? `${selectedColumns.length} columns selected` : "Select key columns..."}
              </span>
              ...
              <SortableContext items={selectedIds} strategy={verticalListSortingStrategy}>
                <div className="space-y-1.5">
                  {selectedColumns.map(col => (
                    col && <SortableKeyItem key={col.id} column={col} onRemove={() => handleSelect(col.id)} />
                  ))}
                </div>
              </SortableContext>
            </DndContext>
