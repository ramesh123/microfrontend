...
import type { FunnelCustomizationOptions } from './funnelCustomizeTypes';

export const FUNNEL_CUSTOMIZE_PANEL_CLASS = 'space-y-1.5 px-1 pb-1 w-full min-w-0 max-w-full overflow-x-hidden box-border';

export function FunnelCustomizePanelShell({
  title,
  description,
  children,
}: {
  ...
  return (
    <div className={FUNNEL_CUSTOMIZE_PANEL_CLASS}>
...
export function FunnelCustomizeFieldRow({
  label,
  hint,
  children,
  className,
...
export function FunnelCustomizeToggleRow({
  label,
  hint,
  checked,
  onCheckedChange,
...
export function FunnelCustomizeCollapsibleSection({
  title,
  description,
  defaultOpen = false,
  children,
...
export function dispatchFunnelCustomizationChange(next: FunnelCustomizationOptions) {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent('chartCustomizationChanged', { detail: next }));
  (window as { __chartCustomizationOptions?: FunnelCustomizationOptions }).__chartCustomizationOptions = next;
}

export type FunnelCustomizeSectionProps = {
  options: FunnelCustomizationOptions;
  onOptionsChange: (next: FunnelCustomizationOptions) => void;
};

export function useFunnelCustomizeHandlers(
  options: FunnelCustomizationOptions,
  onOptionsChange: (next: FunnelCustomizationOptions) => void,
) {
  const update = React.useCallback(
    <K extends keyof FunnelCustomizationOptions>(key: K, value: FunnelCustomizationOptions[K]) => {
      const next = { ...options, [key]: value };
      onOptionsChange(next);
      dispatchFunnelCustomizationChange(next);
    },
    [options, onOptionsChange],
  );

  const applyOptions = React.useCallback(
    (next: FunnelCustomizationOptions) => {
      onOptionsChange(next);
      dispatchFunnelCustomizationChange(next);
    },
    [onOptionsChange],
  ...
  const updateOptions = React.useCallback(
    (patch: Partial<FunnelCustomizationOptions>) => {
