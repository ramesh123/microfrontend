      ...
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {failPercentage != null && !Number.isNaN(failPercentage) && (
...
const NWayValidationTable: React.FC<NWayValidationTableProps> = ({ apiResponse }) => {
  ...
  const summaryKey = useMemo(
    () => apiResponse?.data?.summary_key ?? apiResponse?.summary_key,
    [apiResponse]
  );

  const passPercentage = summaryKey?.pass_percentage;
  const failPercentage = summaryKey?.fail_percentage;
    ...
    <div className="w-full space-y-4 p-2">
      <div className="sticky top-0 z-10 bg-background pb-2 pt-2">
        <div className="flex flex-wrap gap-4 justify-between items-center">
          ...
          <div className="flex flex-wrap gap-3 justify-center items-center">
            ...
            {passPercentage != null && !Number.isNaN(passPercentage) && (
              <Badge variant="outline" className="bg-green-50 text-green-800 px-3 py-1 border-green-200">
                Pass %: {passPercentage}%
              </Badge>
            )}
            {failPercentage != null && !Number.isNaN(failPercentage) && (
              <Badge variant="outline" className="bg-red-50 text-red-800 px-3 py-1 border-red-200">
                Fail %: {failPercentage}%
              </Badge>
            )}
            <Button
      ...
      {filteredRecords && (
        <PairDataGrid
          ...
          failPercentage={failPercentage ?? null}
        />
      )}
    </div>
  );
