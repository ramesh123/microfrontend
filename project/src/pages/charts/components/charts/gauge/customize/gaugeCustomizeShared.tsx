...
import type { GaugeCustomizationOptions } from './gaugeCustomizeTypes';

export const GAUGE_CUSTOMIZE_PANEL_CLASS = 'space-y-1.5 px-1 pb-1 w-full min-w-0 max-w-full overflow-x-hidden box-border';

export function GaugeCustomizePanelShell({
  title,
  description,
  children,
}: {
  ...
  return (
    <div className={GAUGE_CUSTOMIZE_PANEL_CLASS}>
...
export function GaugeCustomizeFieldRow({
  label,
  hint,
  children,
  className,
...
export function GaugeCustomizeToggleRow({
  label,
  hint,
  checked,
  onCheckedChange,
...
export function GaugeCustomizeCollapsibleSection({
  title,
  description,
  defaultOpen = false,
  children,
...
export function dispatchGaugeCustomizationChange(next: GaugeCustomizationOptions) {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent('chartCustomizationChanged', { detail: next }));
  (window as { __chartCustomizationOptions?: GaugeCustomizationOptions }).__chartCustomizationOptions = next;
}

export type GaugeCustomizeSectionProps = {
  options: GaugeCustomizationOptions;
  onOptionsChange: (next: GaugeCustomizationOptions) => void;
};

export function useGaugeCustomizeHandlers(
  options: GaugeCustomizationOptions,
  onOptionsChange: (next: GaugeCustomizationOptions) => void,
) {
  const update = React.useCallback(
    <K extends keyof GaugeCustomizationOptions>(key: K, value: GaugeCustomizationOptions[K]) => {
      const next = { ...options, [key]: value };
      onOptionsChange(next);
      dispatchGaugeCustomizationChange(next);
    },
    [options, onOptionsChange],
  );

  const applyOptions = React.useCallback(
    (next: GaugeCustomizationOptions) => {
      onOptionsChange(next);
      dispatchGaugeCustomizationChange(next);
    },
    [onOptionsChange],
