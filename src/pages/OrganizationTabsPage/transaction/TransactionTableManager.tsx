...
interface TransactionTableManagerProps {
  transaction: Transaction;
  onBack: () => void;
  onUpdateTransaction: (updatedTx: Transaction) => void;
  onViewFields: (table: TransactionTable) => void;
}

export function TransactionTableManager({ transaction, onBack, onUpdateTransaction, onViewFields }: TransactionTableManagerProps) {
  ...
  const handleSave = (tableData: Omit<TransactionTable, 'id' | 'fields'>) => {
    let updatedTables;
    if (editingTable) {
      updatedTables = transaction.tables.map(t => t.id === editingTable.id ? { ...editingTable, ...tableData } : t);
    } else {
      const newTable: TransactionTable = { id: `TBL-${Date.now()}`, ...tableData, fields: [] };
              ...
              <TableRow key={table.id}>
                ...
                <TableCell>
                  <div className="flex justify-center gap-1">
                    <Button variant="accent" size="sm" className="h-8 text-xs" onClick={() => onViewFields(table)}>Fields</Button>
...
function TableForm({ onClose, onSave, initialData }: { 
    onClose: () => void; 
    onSave: (data: Omit<TransactionTable, 'id' | 'fields'>) => void; 
  ...
  useState(() => {
    if (initialData) {
        setFormData({ name: initialData.name, type: initialData.type, description: initialData.description });
    }
  });
  ...
  return (
    <div className="p-4 border rounded-lg space-y-4 bg-muted/50">
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button size="sm" onClick={handleSave} variant="accent">Save Table</Button>
      </div>
    </div>
  );
}
