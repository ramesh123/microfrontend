...
import {
  attachAreaChartCrosshairUi,
  attachAreaChartCursor,
  createAreaCategoryAxisTooltip,
  resolveAreaCurveStyle,
} from '../utils/areaChartCrosshairUi';
import {
...
interface AreaChartProps {
  ...
  customizationOptions?: Partial<AreaCustomizationOptions> | null;
}

export function AreaChart({ data, onChartInteraction, columns, x_axis, metrics: metricsProp = [], customizationOptions }: AreaChartProps) {
  const { theme } = useTheme();
  const chartRef = useRef<HTMLDivElement>(null);
  const rootRef = useRef<am5.Root | null>(null);
  const [areaDomLegend, setAreaDomLegend] = useState<ChartDomScrollLegendItem[]>([]);

  const [options, setOptions] = useState<Partial<AreaCustomizationOptions>>(() => {
    if (customizationOptions) return customizationOptions as any;
    if (typeof window !== 'undefined') return (window as any).__chartCustomizationOptions || areaDefaultOptions;
    return areaDefaultOptions as any;
  });
  ...
  useEffect(() => {
    ...
    const showLegend =
      ...
      Boolean((options && typeof (options as any).showLegend !== 'undefined') ? (options as any).showLegend : true);
    ...
    try {
      ...
      const xAxisFontSize = Number((options && (options as any).xAxisFontSize) ?? (options && (options as any).fontSize) ?? 4);
      xRenderer.labels.template.setAll({ fontSize: xAxisFontSize, rotation: xRot, fill: foregroundColor });
    } catch (e) {
      const xAxisFontSize = Number((options && (options as any).xAxisFontSize) ?? (options && (options as any).fontSize) ?? 4);
      xRenderer.labels.template.setAll({ fontSize: xAxisFontSize, fill: foregroundColor });
    }
    ...
    try {
      if ((options as any)?.truncateXAxis) {
        const minRaw = (options as any)?.xAxisMin ?? '';
        const maxRaw = (options as any)?.xAxisMax ?? '';
    ...
    const areaCurveStyle = resolveAreaCurveStyle(options as AreaCustomizationOptions);
    const areaFillOpacity = Number((options as AreaCustomizationOptions)?.areaFillOpacity ?? 0.28);
    const comparisonSeriesDashed = (options as AreaCustomizationOptions)?.comparisonSeriesDashed !== false;
    const areaCrosshairTooltip = (options as AreaCustomizationOptions)?.areaCrosshairTooltip !== false;
    const lineThickness = resolveLineThickness(
      (options as AreaCustomizationOptions)?.lineThickness,
      DEFAULT_AREA_LINE_THICKNESS,
    );
    const comparisonLineThickness = Math.max(1, lineThickness - 0.25);
    const hasMultipleSeries = sortedSeriesFields.length > 1;

    if (areaCrosshairTooltip) {
      xAxis.set(
        'tooltip',
        createAreaCategoryAxisTooltip(root, am5, foregroundColor, valueLabelBg, gridLineStroke),
      );
    }
    ...
    try {
      ...
      const yAxisFontSize = Number((options && (options as any).yAxisFontSize) ?? (options && (options as any).fontSize) ?? 8);
      yRenderer.labels.template.setAll({ fontSize: yAxisFontSize, rotation: yRot, fill: foregroundColor });
    } catch (e) {
      const yAxisFontSize = Number((options && (options as any).yAxisFontSize) ?? (options && (options as any).fontSize) ?? 8);
      yRenderer.labels.template.setAll({ fontSize: yAxisFontSize, fill: foregroundColor });
    }

    // number format + currency resolution
    const rawNf = (options as any)?.numberFormat as string | undefined;
    ...
    const valueMode = (options as any)?.valueFormatMode || 'adaptive';
    const valueDp = Number.isFinite(Number((options as any)?.valueDecimalPlaces)) ? Number((options as any)?.valueDecimalPlaces) : 2;
    const applyValueFmt = typeof (options as any)?.applyValueDecimalPlaces === 'undefined' ? true : Boolean((options as any)?.applyValueDecimalPlaces);

    const adaptiveFormat = (num: number, dp: number) => {
      if (typeof num !== 'number' || isNaN(num)) return String(num || '0');
      const sign = num < 0 ? '-' : '';
      const abs = Math.abs(num);
      if (abs >= 1000000000) {
        return `${sign}${(abs / 1000000000).toFixed(dp).replace(/\.0+$/, '')}B`;
      }
      if (abs >= 1000000) {
        return `${sign}${(abs / 1000000).toFixed(dp).replace(/\.0+$/, '')}M`;
      }
      if (abs >= 1000) {
        return `${sign}${(abs / 1000).toFixed(dp).replace(/\.0+$/, '')}K`;
      }
      return sign + abs.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: dp });
    };

    const formatUsingOptions = (val: number) => {
      try {
        // Y-Axis Format from UI: numberFormat 'short' (K/M/B) vs 'raw' (Decimal)
        const nfRaw = rawNf || 'short';
        let numericStr: string;
        if (nfRaw === 'raw' || nfRaw === 'decimal' || nfRaw === 'full') {
          numericStr = (val).toLocaleString('en-US', { minimumFractionDigits: valueDp, maximumFractionDigits: valueDp });
        } else if (nfRaw === 'short' || nfRaw === 'adaptive') {
          numericStr = adaptiveFormat(val, valueDp);
        } else if (nfRaw === 'percent') {
          const abs = Math.abs(val);
          const pct = abs <= 1 ? (val * 100) : val;
          numericStr = pct % 1 === 0 ? String(Math.round(pct)) : pct.toFixed(2).replace(/\.0+$/, '');
          return `${numericStr}%`;
        } else {
          numericStr = formatNumber(val, { format: 'short' });
        }
        // Currency Format from UI: none / prefix / suffix with currencyCode symbol
        if (_currencyFmt && _currencyFmt !== 'none') {
          // Prefer Intl.NumberFormat when currency code is available (more robust)
          if (_currencyCode) {
            try {
              const nf = new Intl.NumberFormat('en-US', { style: 'currency', currency: String(_currencyCode) as any });
              if (_currencyFmt === 'prefix') return nf.format(val);
              if (_currencyFmt === 'suffix') {
                // Intl normally renders prefix; for suffix, fall back to symbol or code
                if (_resolvedSymbol) return `${numericStr} ${_resolvedSymbol}`;
                return `${numericStr} ${String(_currencyCode)}`;
              }
            } catch (e) {
              // If Intl isn't available/fails, fall through to symbol fallback
            }
          }
          if (_resolvedSymbol) {
            if (_currencyFmt === 'prefix') return `${_resolvedSymbol} ${numericStr}`;
            if (_currencyFmt === 'suffix') return `${numericStr} ${_resolvedSymbol}`;
          }
        }
        return numericStr;
      } catch (e) {
    ...
    yRenderer.labels.template.adapters.add('text', (text, target) => {
      if (text && typeof text === 'string') {
        ...
        const numMatch = cleanedText.match(/^[\d.-]+$/);
          ...
          if (!isNaN(numValue)) {
            return formatUsingOptions(numValue);
          }
        }
      }
      return text || '';
    ...
    const logRequested = Boolean((options as any)?.logarithmicAxis);
    ...
    // Area/line colours: neatChartHexAtIndex + shine gradients (am5MiniChartLine)

    // Create series for each field
    const lineSeriesArray: Array<am5xy.SmoothedXLineSeries | am5xy.LineSeries> = [];
    let primarySeriesRef: am5xy.SmoothedXLineSeries | am5xy.LineSeries | null = null;
    let primaryFieldRef = sortedSeriesFields[0] ?? 'value';
    let primaryColorRef = am5.color(0x16a34a);
    ...
    sortedSeriesFields.forEach((field, index) => {
      ...
      const isComparisonSeries = hasMultipleSeries && comparisonSeriesDashed && index > 0;

      const lineTooltip = areaCrosshairTooltip
        ? undefined
        : createSeriesLinkedTooltipWithText(
            ...
            (di) => {
              ...
              const resolvedVy = Number.isFinite(vy) ? vy : Number(ctx?.[field]);
              if (!Number.isFinite(resolvedVy)) return '';
              const formattedValue = formatUsingOptions(resolvedVy);
      ...
      const seriesCommon = {
        ...
        ...(lineTooltip ? { tooltip: lineTooltip as unknown as am5.Tooltip } : {}),
      };

      const series =
        areaCurveStyle === 'linear'
      ...
      if (index === 0) {
        primarySeriesRef = series;
        primaryFieldRef = field;
        primaryColorRef = midColor;
      }
      ...
      try {
        ...
        const showGradientFill = !isComparisonSeries;
        const comparisonStroke = chartTheme.muted;
        series.strokes.template.setAll({
          strokeWidth: isComparisonSeries ? comparisonLineThickness : lineThickness,
          strokeGradient: isComparisonSeries ? undefined : strokeGradient,
          stroke: isComparisonSeries ? comparisonStroke : midColor,
          strokeOpacity: isComparisonSeries ? 0.85 : 0.95,
          ...(isComparisonSeries ? { strokeDasharray: [6, 4] } : {}),
          interactive: true,
        });
        series.set('stroke', isComparisonSeries ? comparisonStroke : midColor);
        series.set('fill', midColor);
        series.fills.template.setAll({
          visible: showGradientFill,
          fillOpacity: showGradientFill ? areaFillOpacity : 0,
          fillGradient: showGradientFill ? areaFillGradient : undefined,
        });
        ...
        series.fills.template.setAll({
          fillOpacity: !isComparisonSeries ? areaFillOpacity : 0,
          visible: !isComparisonSeries,
        });
      }

      if (!areaCrosshairTooltip && lineTooltip) {
        series.bullets.push(() => {
              ...
              try {
                ...
                if (typeof onChartInteraction === 'function') {
                  onChartInteraction(
                    ...
                    payload?.eventDimensionFields,
                    payload?.eventDrillFilters,
                  );
                }
      ...
      try {
        const showLabels = Boolean(
          (options && typeof (options as any).showValue !== 'undefined') ? (options as any).showValue : false,
        );
        if (showLabels) {
          series.bullets.push((root, _series, dataItem) => {
            const label = am5.Label.new(root, {
              ...
              fontSize: (options as any)?.fontSize || 12,
              textAlign: 'center',
              paddingLeft: 6,
              paddingRight: 6,
              paddingTop: 2,
            ...
            try {
              ...
              label.set('text', formatUsingOptions(Number(v) || 0));
            } catch {
              /* ignore */
            }
    ...
    if (lineSeriesArray.length > 0) {
      attachAreaChartCursor(chart, root, am5, xAxis, yAxis, lineSeriesArray, gridLineStroke);
      if (areaCrosshairTooltip && primarySeriesRef) {
        attachAreaChartCrosshairUi({
          root,
          am5lib: am5,
          chart,
          xAxis,
          yAxis,
          primarySeries: primarySeriesRef,
          primaryField: primaryFieldRef,
          categoryField,
          seriesColor: primaryColorRef,
          foregroundColor,
          cardBg: valueLabelBg,
          borderColor: gridLineStroke,
          formatValue: formatUsingOptions,
          allRows: effectiveChartData as Array<Record<string, unknown>>,
        });
      }
    }
    ...
    try {
      ...
      if ((options as any)?.dataZoom && effectiveChartData.length > dataZoomMin) {
    ...
    } else if ((options as any)?.animation === false) {
      chart.appear(0, 0);
    } else {
  ...
  return (
    <div className={`relative flex h-full w-full min-h-0 ${
      ...
      <ChartDomScrollLegend
        visible={areaDomLegend.length > 0}
        items={areaDomLegend}
