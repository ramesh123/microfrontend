...
export const PIE_CUSTOMIZE_PANEL_CLASS = 'space-y-1.5 px-1 pb-1';

export function PieCustomizePanelShell({
  title,
  description,
  children,
}: {
  ...
  return (
    <div className={PIE_CUSTOMIZE_PANEL_CLASS}>
...
export function PieCustomizeFieldRow({
  label,
  hint,
  children,
  className,
...
export function PieCustomizeToggleRow({
  label,
  hint,
  checked,
  onCheckedChange,
...
export function PieCustomizeCollapsibleSection({
  title,
  description,
  defaultOpen = false,
  children,
...
export function dispatchPieCustomizationChange(next: ChartCustomizationOptions) {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent('chartCustomizationChanged', { detail: next }));
  (window as { __chartCustomizationOptions?: ChartCustomizationOptions }).__chartCustomizationOptions = next;
}

export type PieCustomizeSectionProps = {
  options: ChartCustomizationOptions;
  onOptionsChange: (next: ChartCustomizationOptions) => void;
};

export function usePieCustomizeHandlers(
  options: ChartCustomizationOptions,
  onOptionsChange: (next: ChartCustomizationOptions) => void,
) {
  const update = React.useCallback(
    <K extends keyof ChartCustomizationOptions>(key: K, value: ChartCustomizationOptions[K]) => {
      const next = { ...options, [key]: value };
      onOptionsChange(next);
      dispatchPieCustomizationChange(next);
    },
    [options, onOptionsChange],
  );

  const applyOptions = React.useCallback(
    (next: ChartCustomizationOptions) => {
      onOptionsChange(next);
      dispatchPieCustomizationChange(next);
    },
    [onOptionsChange],
  ...
  const updateOptions = React.useCallback(
    (patch: Partial<ChartCustomizationOptions>) => {
