...
import type { SunburstCustomizationOptions } from './sunburstCustomizeTypes';

export const SUNBURST_CUSTOMIZE_PANEL_CLASS = 'space-y-1.5 px-1 pb-1';

export function SunburstCustomizePanelShell({
  title,
  description,
  children,
}: {
  ...
  return (
    <div className={SUNBURST_CUSTOMIZE_PANEL_CLASS}>
...
export function SunburstCustomizeFieldRow({
  label,
  hint,
  children,
  className,
...
export function SunburstCustomizeToggleRow({
  label,
  hint,
  checked,
  onCheckedChange,
...
export function SunburstCustomizeCollapsibleSection({
  title,
  description,
  defaultOpen = false,
  children,
...
export function dispatchSunburstCustomizationChange(next: SunburstCustomizationOptions) {
  if (typeof window === 'undefined') return;
  window.dispatchEvent(new CustomEvent('chartCustomizationChanged', { detail: next }));
  (window as { __chartCustomizationOptions?: SunburstCustomizationOptions }).__chartCustomizationOptions = next;
}

export type SunburstCustomizeSectionProps = {
  options: SunburstCustomizationOptions;
  onOptionsChange: (next: SunburstCustomizationOptions) => void;
};

export function useSunburstCustomizeHandlers(
  options: SunburstCustomizationOptions,
  onOptionsChange: (next: SunburstCustomizationOptions) => void,
) {
  const update = React.useCallback(
    <K extends keyof SunburstCustomizationOptions>(key: K, value: SunburstCustomizationOptions[K]) => {
      const next = { ...options, [key]: value };
      onOptionsChange(next);
      dispatchSunburstCustomizationChange(next);
    },
    [options, onOptionsChange],
  );

  const applyOptions = React.useCallback(
    (next: SunburstCustomizationOptions) => {
      onOptionsChange(next);
      dispatchSunburstCustomizationChange(next);
    },
    [onOptionsChange],
  ...
  const updateOptions = React.useCallback(
    (patch: Partial<SunburstCustomizationOptions>) => {
