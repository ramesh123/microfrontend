...
export const useShortcutsStore = create<shortcutsStoreType>((set: any, get: any) => ({
  shortcuts: defaultShortcuts,
  setShortcuts: (newShortcuts: typeof defaultShortcuts) => {
  ...
  updateUniqueShortcut: (name: string, combination: string) => {
    set({
      [name]: combination,
    });
  },
  getShortcutsFromStorage: () => {
    if (localStorage.getItem("langflow-shortcuts")) {
      ...
      savedArr.forEach(({ name, shortcut }: { name: string; shortcut: string }) => {
